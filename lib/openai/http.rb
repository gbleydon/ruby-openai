...
module OpenAI
  module HTTP
    ...
    def get(path:, parameters: nil)
      parse_jsonl(conn.get(uri(path: path,params: parameters), parameters) do |req|
        req.headers = headers
      end&.body)
    end
    ...
    def json_post(path:, parameters:, query_parameters: {})
      conn.post(uri(path: path, params: parameters)) do |req|
    ...
    def multipart_post(path:, parameters: nil)
      conn(multipart: true).post(uri(path: path, params: parameters)) do |req|
    ...
    def build_azure_uri(path:,params:)
      case path
        when /\/chat\/completions\z/
          base = File.join(@uri_base,'deployments',"#{params[:model]}","chat")
        when /\/completions\z/
    ...
    def uri(path:,params: nil)
      if azure?
        build_azure_uri(path: path,params: params)
